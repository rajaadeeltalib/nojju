...
const getSleepingBagPosts = async () => {
  const res = await client.fetch(`*[_type=='posts' && category=='CAMPING SLEEPING BAG'] | order(date desc)`);
...
const SleepingBag = async() => {
  const blogsPosts = await getSleepingBagPosts();
  return (
